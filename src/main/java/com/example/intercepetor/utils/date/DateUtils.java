package com.example.intercepetor.utils.date;
 ...
 * @Author: XueWeiDong
 * @Description:
 * @Date: 15:53 2019/12/23
 */
public class DateUtils {
    private static final String extractPatterns = StringUtils.join(new String[] { "([>|<]?\\s*\\d{4}-\\d{1,2}-\\d{1,2} \\d{1,2}:\\d{1,2}:\\d{1,2})", //yyyy-MM-dd hh:mm:ss
            "([>|<]?\\s*\\d{4}-\\d{1,2}-\\d{1,2})", //yyyy-MM-dd
            "([>|<]?\\s*\\d{4}-\\d{1,2})", //yyyy-MM
            "([>|<]?\\s*\\d{4})" //yyyy
    }, "|");
    ...
    public static String timestamp() {
        return getFormatCurrentTime("yyyyMMddHHmmss");
    }

    public static String[] extractDateParams(String query) {
        if (StringUtils.isBlank(query)) {
            return new String[0];
        }
        Pattern pattern = Pattern.compile(extractPatterns);
    ...
    public static String dateFormat2(String sdate, String format) {
        try {
            SimpleDateFormat formatter = new SimpleDateFormat(format);
    ...
    public static String getFormatTime1(Date date) {
        return getFormatDateTime(date, "yyyyMMddHHmm");
    }

    public static String getFormatTime(long date) {
        return getFormatDateTime(new Date(date), "yyyy-MM-dd HH:mm:ss");
    }
    ...
    public static Date getTheEarliestDate(Collection<Date> dates) {
        ...
        for (Date date : dates) {
            ...
            if (earliestDate.getTime() > date.getTime()) {
                earliestDate = date;
            }
        }
        return earliestDate;
    ...
    public static Date getTheLatestDate(Collection<Date> dates) {
    ...
    /**
     * 自动返回距离现在时间相近几年的时间。例如当前年份是2007年，那么就自动返回2007+yearInterval
     *
     * @return 返回结果的格式为 yyyyMMddHHmmss
     */
    public static String getCurrentTimeIntervalTime(int yearInterval) {
        String currentYear = getFormatCurrentTime("yyyy");
        String currentTime = getFormatCurrentTime("MMddHHmmss");
        int beforeYear = Integer.parseInt(currentYear) + yearInterval;
        return "" + beforeYear + currentTime;
    }

    /**
     * 获取某时间 前N个月 的 日期
     * @param timestamp 时间戳 格式20150720
     * @param amount  N (N为正数表示加上月份，为负数表示减去月份)
     * @return
     */
    public static String getSpaceMonth(String timestamp, int amount, String datePattern) {
        SimpleDateFormat sfd = new SimpleDateFormat(datePattern);
        Calendar c = null;
        try {
            Date date = sfd.parse(timestamp);
    ...
    /**
     * 传入星期几，获取当前日期到 当年年底 例如（20150812 ~ 20151231） 所有星期 几的日期集合
     * @param week
     * @return
     */
    public static List<String> getDaysByWeek(int week) {
        ...
        nexty.set(nextyear, Calendar.UNDECIMBER - 1, 31);//月的索引是从0开始
        int dayOfweek = nexty.get(Calendar.DAY_OF_WEEK) - 1;
        if (dayOfweek != week) {
            int newDay = dayOfweek - week;
            if (dayOfweek < week) {
                newDay = (dayOfweek + Calendar.SATURDAY) - week;
            }
            nexty.add(Calendar.DAY_OF_YEAR, -newDay);
        }

        List<String> dates = new ArrayList<String>();
        for (; nexty.after(calendar) || nexty.equals(calendar); nexty.add(Calendar.DAY_OF_YEAR, -7)) {
            int year = calendar.get(Calendar.YEAR);
            int month = 1 + nexty.get(Calendar.MONTH);
            int day = nexty.get(Calendar.DATE);
            //			System.out.println(nexty.get(Calendar.YEAR) + "-" + (1 + nexty.get(Calendar.MONTH))
            //					+ "-" + nexty.get(Calendar.DATE));
            String newMonth = month < 10 ? "0" + month : month + "";
            String newDay = day < 10 ? "0" + day : day + "";
            dates.add(String.valueOf(year) + newMonth + newDay);
     ...
     * @param nowYear
     */
    public static List<String> getDaysByYear(int nowYear) {
        Calendar calendar = Calendar.getInstance();//当前日期
        Calendar nexty = Calendar.getInstance();//当前日期
        calendar.set(nowYear, 11, 31);
        nexty.set(nowYear, 0, 0);
        List<String> dates = new ArrayList<String>();
        for (; nexty.before(calendar); calendar.add(Calendar.DAY_OF_YEAR, -1)) {
            int year = calendar.get(Calendar.YEAR);
            int month = 1 + calendar.get(Calendar.MONTH);
            int day = calendar.get(Calendar.DATE);
            //			System.out.println(calendar.get(Calendar.YEAR) + "-"
            //					+ (1 + calendar.get(Calendar.MONTH)) + "-" + calendar.get(Calendar.DATE));
            String newMonth = month < 10 ? "0" + month : month + "";
            String newDay = day < 10 ? "0" + day : day + "";
            dates.add(String.valueOf(year) + newMonth + newDay);
        }
    ...
    /**
     * String 日期 转换为另外格式 String
     * @param beFormatDate 被转日期
     * @param beFormat 被转格式
     * @param resultFormat 结果格式
     * @return
     */
    public static String strDateToStr(String beFormatDate, String beFormat, String resultFormat) {
        try {
            SimpleDateFormat formatter = new SimpleDateFormat(beFormat);
            Date date = formatter.parse(beFormatDate);
            return getFormatDateTime(date, resultFormat);
        } catch (ParseException e) {
            e.printStackTrace();
        }
        return null;
